...
- **Runs as a persistent Node server** (`output: 'server'`, `@astrojs/node` standalone), not a static site — needs a host that runs Node (or the Vercel adapter), not just static hosting (unlike the storefront).
